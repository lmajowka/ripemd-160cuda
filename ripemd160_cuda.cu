...
int main() {
    // Ajustado de uma entrada utf-8 para hex com entrada de 33 bytes (chave comprimida)
    const char* input_sha256 = "e846c2c3a5c927c9d23d5aa249c838a91256174db96c61c9e5d340aaae9962e6";
    unsigned char byte_array[32];

    // Converta a string hex para bytes
    for (int i = 0; i < 32; i++) {
        sscanf(input_sha256 + 2*i, "%2hhx", &byte_array[i]);
    }
    ...
    size_t input_len = sizeof(byte_array);
    
    cudaMalloc((void**)&d_data, input_len);
    cudaMalloc((void**)&d_hash, RIPEMD160_DIGEST_SIZE);

    // Copy data to GPU
    cudaMemcpy(d_data, byte_array, input_len, cudaMemcpyHostToDevice);

    // Launch kernel to compute SHA-256
    ripemd160_gpu<<<1, 1>>>(d_data, input_len, d_hash);
